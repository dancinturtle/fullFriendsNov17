...
class MySQLConnection(object):
    def __init__(self, app, friendsdb):
        config = {
                'host': 'localhost',
                'database': friendsdb, # we got friendsdb as an argument
                'user': 'root',
                'password': 'root',
    ...
    def query_db(self, query, data=None):
        ...
        if query[0:6].lower() == 'select':
            ...
            self.friendsdb.session.commit()
# This is the module method to be called by the user in server.py. Make sure to provide the friendsdb name!
